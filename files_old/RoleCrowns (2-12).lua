...
UICrowns = {}
local crowns = UICrowns
-- local crown_control, crown_texture, crown_texture2
...
function Initialize()
	...
	-- Keep checking group size, if it changes then call update again
	-- StartSizePoll()
		
	local loop_log = "group_loop"
	EVENT_MANAGER:RegisterForUpdate(loop_log, 30000, LogGroupData)
...
-- the function call to check roles fails during bgs
...
function DrawNewRoleCrown(index)
	...
	crown_texture2 = WINDOW_MANAGER:CreateControl(nil, crown_control, CT_TEXTURE)
	
	-- Make the control 3 dimensional
	crown_control:Create3DRenderSpace()
	crown_texture:Create3DRenderSpace()
	crown_texture2:Create3DRenderSpace()
	...
	crown_texture2:SetTexture(ICON_PATH .. "/healer_simple.dds")
	crown_texture2:Set3DLocalDimensions(.65, .65)
	crown_texture2:Set3DRenderSpaceUsesDepthBuffer(true)
	crown_texture2:Set3DRenderSpaceOrigin(0,0,0)	
	
	-- Set RGB between 0 and 1 (overrides alpha value to 1)
	-- Can set alpha with its 4th parameter
	crown_texture:SetColor(1.0, 0.0, 0.0)  
	crown_texture2:SetColor(1.0, 0.0, 0.0)  

	-- Set alpha between 0 and 1
	crown_texture:SetAlpha(0.5)
	crown_texture2:SetAlpha(0.5)
end
...
function ShowCrownUI()
	...
	crown_texture2:SetHidden(false)
	...
	EVENT_MANAGER:RegisterForUpdate("CrownUI", 0, function(time)
		...
		RoleCrown_TopLevel:Set3DRenderSpaceOrigin(rc_obj.group_list[2].x, rc_obj.group_list[2].y, rc_obj.group_list[2].z)
...
function HideCrownUI()
	...
	crown_control:SetHidden(true)
	crown_texture:SetHidden(true)
	crown_texture2:SetHidden(true)
end
